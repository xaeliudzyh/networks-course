...
1. Поверх какого протокола посылаются сообщения DHCP – UDP или TCP?
   - UDP(User Datagram Protocol, Src Port: 68, Dst Port: 67)
...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - Transaction ID: 0x4ee04244 у обоих
   - Назначение - так хосты при броадкасте понимают, что сервер им отвечает 
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - Source Address: 0.0.0.0, Source Address: 0.0.0.0
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   ...
   - 6 часов, чтобы, чтобы с периодом в это время сбрасывать IP-адреса устройствам, которые больше не подключены к сети
