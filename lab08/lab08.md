...
#### Решение
1. В «Congestion Avoidance» каждое RTT окно увеличивается на 1 MSS:
   $
     \mathrm{cwnd}(k) \;=\; \tfrac{W}{2} + k \quad\text{после }k\text{ RTT.}
   $
2. Чтобы дойти от $\tfrac{W}{2}$ до $W$, нужно $k = \tfrac{W}{2}$. Значит
   $
     T \;=\; \frac{W/2}{1} \;=\; \frac{W}{2} \quad(\text{RTT}).
   $
3. Средняя скорость TCP (в MSS/RTT) в steady state ≈ $\frac{3W/4}{\text{RTT}}$. Обозначим $\lambda =$ throughput (MSS/RTT). Тогда
   $
     \lambda = \frac{3W/4}{\mathrm{RTT}} 
     \;\Longrightarrow\; W = \frac{4}{3}\,\lambda\,\mathrm{RTT}.
   $
   Подставляем в $T = \tfrac{W}{2}$:
   $
     T = \frac{1}{2}\,\frac{4}{3}\,\lambda\,\mathrm{RTT} 
       = \frac{2}{3}\,\lambda\,\mathrm{RTT}.
   $
   **Итого:**  
   $
     \boxed{T = \frac{W}{2} = \frac{2}{3}\,\lambda\,\mathrm{RTT},}
   $
   т. е. ЧТД
...
#### Решение
1. **Client↔Edge handshake** = 1 RTT$_{FE}$.  
2. **Slow start (client → edge)**:  
   - RTT$_{FE}^1$: 1 MSS → ACK,  
   - RTT$_{FE}^2$: 2 MSS → ACK,  
   - RTT$_{FE}^3$: 4 MSS (запрос HTTP) → ACK.  
   Итого 3 RTT$_{FE}$.  
3. Как только Edge получил запрос (≪ 4 MSS), он **мгновенно** пересылает его в DC (ОКНО EDGE→DC = большое, без slow start).  
4. DC обрабатывает запрос, отвечает → возвращает ответ Edge за 1 RTT$_{BE}$.  
5. Edge, получив ответ, отправляет клиенту (client↔edge) за 1 RTT$_{FE}$.  

Суммируем:
- 1 RTT\(_{FE}\) (handshake)  
- 3 RTT\(_{FE}\) (slow start)  
- 1 RTT\(_{BE}\) (DC обрабатывает и отсылает ответ)  
- 1 RTT\(_{FE}\) (доставка ответа клиенту)  

$
  \boxed{4\,RTT_{FE} \;+\; RTT_{BE} \;+\; \text{(время обработки).}}
$
